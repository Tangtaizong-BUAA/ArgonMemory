 ...
 *   2. ~/.config/argon-memory/doc-reading.json
 *   3. ./argon-memory.config.json (cwd)
 *
 * Example argon-memory.config.json:
 * {
 *   "docReading": {
 *     "providers": {
...
export function getDocReadingConfig(): DocReadingConfig {
  ...
  const homeConfigPath = join(xdgConfig, "argon-memory", "normalization.json");
  const cwdConfigPath  = resolve(process.cwd(), "argon-memory.config.json");
